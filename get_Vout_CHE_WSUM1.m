function [sprecoder,VoutUser,Vout] =  get_Vout_CHE_WSUM1(ChannelInfo,TransceiverInfo,InitialM)
...
while(Loop)
 ...
 %   for iSubbandNumber = 1:subbandNumber
    ...
    Vout = 0;
    for iUser = 1:K
        VoutUser(iUser) = b2 *  MrPower * CHE^2 * powerAllocatePuser(: ,iUser)' *  Mdiag(:,:,1) *  powerAllocatePuser(: ,iUser) + 1.5 * b4 * MrPower^2 * CHE^4 * norm( powerAllocatePuser(: ,iUser)' * Mdiag(:,:,1) *  powerAllocatePuser(: ,iUser))^2;
        ...
        if iSubbandNumber ~=1
            Vout = Vout + 3 * b4 * MrPower^2 * CHE^4 * norm( powerAllocatePuser(: ,iUser)' * Mdiag(:,:,iSubbandNumber) *  powerAllocatePuser(: ,iUser))^2;
             VoutUser(iUser) = VoutUser(iUser) +  3 * b4 * MrPower^2 * CHE^4 * norm( powerAllocatePuser(: ,iUser)' * Mdiag(:,:,iSubbandNumber) *  powerAllocatePuser(: ,iUser))^2;
        end
        end
    end
