function [sprecoder,VoutUser,Vout] =  get_Vout_CHE_MAX_MIN_RR1(ChannelInfo,TransceiverInfo,InitialM)
...
%        cvx_solver Mosek
   ...
   nTerm = 0;
    ...
    %Update X
   powerAllocatePuser = get_RR_MAX_MIN_procedure(powerAllocateXstar,App1(:,:,userIndex),Aml,bm,K,MrPower,subbandNumber,Mt);
 ...
 %   for iSubbandNumber = 1:subbandNumber
  ...
  % end
    ...
    if (norm((powerAllocatePuser-powerAllocatePuserlast),'fro')/norm(powerAllocatePuser,'fro')) <= Tolerance/10 || Loop>=50
        
        break;
    else
         Loop = Loop+1;
        
        powerAllocatePuserlast = powerAllocatePuser;
    end
end
    VoutUser  = zeros(K,1);
    Vout = 0;
