...
while(Loop) 
   ...
   for iT = 1:T
       ...
       powerAllocateXGroup(:,:,iT) = vect(:,iT) * vect(:,iT)';
       for iUser =  1:K
           gammaGroup(iUser,iT)  =   trace(Aq1(:,:,iUser) * powerAllocateX) + cqbar(iUser);
       end
       negGamma(iT) = min(gammaGroup(:,iT));
   end
   [negGammaMin,Index] = min(negGamma);
