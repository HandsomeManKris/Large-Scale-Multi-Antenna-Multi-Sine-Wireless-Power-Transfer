function InitialM = get_compact_CHE_WSUM1(TransceiverInfo, ChannelInfo)
...
CHEdiag =zeros(K * subbandNumber,K * subbandNumber);
...
for iUser = 1:K
    for iSubbandNumber = 1:subbandNumber
        ...
        CHEdiag((iUser-1) * subbandNumber + 1:(iUser-1) * subbandNumber + subbandNumber,(iUser-1) * subbandNumber + 1:(iUser-1) * subbandNumber + subbandNumber) = ...
            diag(ones(subbandNumber,1)) * mean( CHEMatrix(:,iUser)); 
       % CHEdiag((iUser-1) * subbandNumber + 1:(iUser-1) * subbandNumber + subbandNumber,(iUser-1) * subbandNumber + 1:(iUser-1) * subbandNumber + subbandNumber) = ...
       %     diag(ones(subbandNumber,1)) * mean( CHEMatrix(iSubbandNumber,iUser)); 
    end
end
