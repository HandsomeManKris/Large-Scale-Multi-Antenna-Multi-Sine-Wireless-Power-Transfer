...
function [sprecoder,pstar,Vout] =  get_Vout_WSUM_S(ChannelInfo,TransceiverInfo,InitialM)




wbeamforming = InitialM.wbeamforming;
...
while(Loop)
    ...
    %computeCpp1
    App1 = zeros(subbandNumber ,subbandNumber );
    ...
    for iUser = 1:K
        ...
        for iSubbandNumber = 1:subbandNumber
            sprecoder((iSubbandNumber-1)*Mt+1:(iSubbandNumber-1)*Mt+Mt,1) = pstar( iSubbandNumber) * wbeamforming(:,iSubbandNumber);
        end
end